...
/**
 * Resolve as features de CLASSE do objeto de classe. Também resolve os
 * `refClassFeature` embutidos no texto (ex: as opções do Divine Order/Primal
...
function classFeatureItems(db, classId, classObj) {
  ...
  // Inlina refClassFeature (sub-features de opções) no corpo, recursivamente.
  const seen = new Set();
...
/**
 * Uma feature é "LONGA" (lista de opções - invocations, maneuvers, metamagic,
 * fighting styles/flourishes…)? Usado p/ começar colapsada na UI ("Options").
 * As features de subclasse já vêm SEPARADAS (subclassFeatureList), então só as
 * que carregam um bloco `options` (ou são longas) começam fechadas.
 */
