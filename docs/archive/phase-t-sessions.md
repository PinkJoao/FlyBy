# Phase T - session hand-off archive

> The 42 session hand-offs that used to live in `TESTING-PLAN.md` §7, from the
> campaign's opening (2026-07-15) to the close of the deferred-review levas
> (2026-07-30). **History only** - the current hand-off lives in TESTING-PLAN §7,
> and the per-unit state of the campaign is `testing/COVERAGE.md`.
>
> Kept because each entry records what was actually executed to certify a batch
> (which probe, which oracle, what was checked live) - useful when a certified
> unit later turns out to be wrong and you need to know how thin the evidence was.

---

## Hand-offs (newest first)
