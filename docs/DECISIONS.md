# Design Decision Log (full archive)

> The complete ADR-style log, DDL-0001 to DDL-0082. **This file is NOT read at the
> start of a session** - `CLAUDE.md` carries the distilled rules plus a one-line
> index of every entry. Come here when the index says an entry is worth opening:
> for the reasoning behind a decision, the alternatives that were rejected, or the
> measurements that settled it.
>
> Entries are newest first. Never silently overwrite a past decision - supersede it
> with a new dated entry, and mark the old one in the index.
...
**Date:** 2026-07-29
**Contexto:** a revisão dos itens adiados por conveniência (`docs/archive/deferred-review.md`), em três levas.
...
- Verificado ao fim das quatro levas: 1274 testes, lint, `npm run check:keys` (0 mortas), sweep
  **286/286** `--strict`, `npm run t2` de 71 → **18** (+65 nomeados como esperados), com `traits.dr`,
  ...
  §103-106 e `docs/archive/deferred-review.md`, reestruturado em PENDENTE / RESOLVIDO / DECIDIDO POR DESIGN / FORA
...
grupo novo vivem em **`docs/archive/species-families-census.md`** (raiz) — não refazer o levantamento.
